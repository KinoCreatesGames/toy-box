...
class FnMacros {
	...
	public static macro function destruct(typeRef:Expr, objectRef:Expr) {
		...
		switch (objectRef.expr) {
			...
			case _:
				return
					Context.error('Incorrect type, must be constant identifier',
					Context.currentPos());
		}
			...
			switch (trest.follow()) {
				case TAnonymous(_.get() => tr):
					...
					for (field in tr.fields) {
						...
						all.set(fname,
							{field: fname, expr: macro $extVar.$fname});
					}
				default:
					return Context.error("Object type expected instead of "
						+ trest.toString(),
	...
	public static macro function jsPatch(prototype:Bool = true,
		...
		if (fields != null) {
			fields.get().iter((classField) -> {
				...
				if (isRType) {
					classFieldRealName = classField.name.substring(0,
						classField.name.length);
				} else {
